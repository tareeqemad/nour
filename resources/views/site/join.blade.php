...
@push('scripts')
    <script>
        // ✅ validatePalID مطابق لمنطق Oracle ID_No (check digit)
        function validatePalID(t) {
            ...
            // positions 1..8 (أول 8 أرقام)
            for (let x = 1; x <= 8; x++) {
                const y = Number(s[x - 1]);

                if (x % 2 === 1) {
                    fn += y; // positions الفردية: جمع مباشر
                } else {
                    let sn = y * 2; // positions الزوجية: ضرب *2
                    if (sn >= 10) sn = Math.floor(sn / 10) + (sn % 10); // جمع رقمين لو صار >=10
                    fn += sn;
                }
            }

            const lid = (10 - (fn % 10)) % 10; // رقم التحقق
            return Number(s[8]) === lid; // آخر رقم لازم يطابق
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                const ownerId = ownerIdNumberInput.value.trim();
                const operatorId = operatorIdNumberInput.value.trim();

                // owner_id_number إلزامي
                if (!validatePalID(ownerId)) {
                    e.preventDefault();
                    alert('رقم هوية المالك غير صحيح. تأكد أنه 9 أرقام صحيحة.');
                    ownerIdNumberInput.focus();
                    return;
                }

                // operator_id_number اختياري: إذا موجود لازم يكون صحيح
                if (operatorId.length > 0 && !validatePalID(operatorId)) {
                    e.preventDefault();
                    alert('رقم هوية المشغل غير صحيح. تأكد أنه 9 أرقام صحيحة أو اتركه فارغاً.');
                    operatorIdNumberInput.focus();
                    return;
                }

                // منع الإرسال المزدوج بعد نجاح التحقق
