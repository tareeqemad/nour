{{-- Sidebar للمشرفين: SuperAdmin و Admin --}}
@php
    $u = auth()->user();
...
{{-- المشغلين (SuperAdmin فقط) - جزء من إدارة النظام --}}
@if($u->isSuperAdmin())
...
@if($u->isSuperAdmin())
    @php
        ...
        $canViewConstants = true;
        $canViewVersions = true;
        $canViewLogs = $u->hasPermission('logs.view');
        $canViewSystemSettings = $canViewSettings || $canViewConstants || $canViewVersions || $canViewLogs;
    ...
    @if($canViewSystemSettings)
        <li class="slide has-sub {{ $isSystemSettingsOpen ? 'open' : '' }}">
            ...
            <ul class="slide-menu child1" style="{{ ($show('admin.settings.*') || $show('admin.constants.*') || $show('admin.versions.*') || $show('admin.logs.*')) ? 'display:block' : '' }}">
                ...
                @if($canViewVersions)
                    <li class="slide">
                        <a href="{{ route('admin.versions.index') }}" class="side-menu__item {{ $isActive('admin.versions.*') }}">
                            إدارة الإصدارات
                        </a>
                    </li>
                @endif
...
{{-- الرسائل (الترحيبية + قوالب SMS) - SuperAdmin و Admin فقط --}}
@php
