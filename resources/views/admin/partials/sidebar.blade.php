    ...
    <div class="main-sidebar" id="sidebar-scroll">
        <nav class="main-menu-container nav nav-pills flex-column sub-open">
            ...
            @php
                ...
                // ══════════ بوابات الصلاحيات (محفوظة كما كانت) ══════════
                $sysRoles = $u->isSuperAdmin() || $u->isAdmin() || $u->isEnergyAuthority();
                ...
                // المشغّلون
                $opVisible = ($u->isSuperAdmin() || $u->isEnergyAuthority()) && $u->can('viewAny', App\Models\Operator::class);
                $showOperatorsSection = $opVisible
                    || $u->can('viewAny', App\Models\AuthorizedPhone::class)
                    || $u->hasPermission('portal_requests.view')
                    || $u->hasPermission('portal_archive.view');

                // قسم العمليات (نفس منطق السابق تماماً)
                ...
                // التوليد والتشغيل
                $canUnits = $u->can('viewAny', App\Models\GenerationUnit::class);
                $canGenerators = $u->can('viewAny', App\Models\Generator::class);
                $canTasks = $u->can('viewAny', App\Models\Task::class);
                ...
                $showGenOps = $showOpsCategory && ($canUnits || $canGenerators || $canViewRecords || $canTasks);

                // المشتركون والفوترة
                $subsAllowed = !$u->isCompanyOwner() || $u->hasApprovedOperator();
                ...
                $canTariff = $u->can('viewAny', App\Models\ElectricityTariffPrice::class);
                $canEmpDiscount = $u->isSuperAdmin() || $u->hasPermission('employee_discount_rates.view');
                $showSubsData = $subsAllowed && ($canSubscribers || $canInvoices || $canSubAccount || $canInvoiceReports);
                $showBillingSettings = $canTariff || $canEmpDiscount || ($subsAllowed && $canMinCharge);
                $showBillingSection = $showOpsCategory && ($showSubsData || $showBillingSettings);
                $billingSettingsOpen = $isOpen('admin.electricity-tariff-prices.*') || $isOpen('admin.operators.tariff-prices.*') || $isOpen('admin.employee-discount-rates.*') || $isOpen('admin.minimum-charge-rules.*');
                $billingSettingsActive = $isActive('admin.electricity-tariff-prices.*') || $isActive('admin.operators.tariff-prices.*') || $isActive('admin.employee-discount-rates.*') || $isActive('admin.minimum-charge-rules.*');
            @endphp
            ...
            <ul class="main-menu">
                ...
                {{-- ══════════════════════════════════════
                   المشغل — CompanyOwner فقط (إدارته الذاتية)
                   ══════════════════════════════════════ --}}
                @if($u->isCompanyOwner())
                ...
                {{-- ══════════════════════════════════════
                   المشغّلون والترخيص — SuperAdmin/Admin/EnergyAuthority
                   ══════════════════════════════════════ --}}
                @if($sysRoles && $showOperatorsSection)
                <li class="slide__category mt-3">
                    <span class="side-menu__label text-muted text-xs opacity-70">المشغّلون والترخيص</span>
                </li>
                ...
                @if($showGenOps)
                <li class="slide__category mt-3">
                    <span class="side-menu__label text-muted text-xs opacity-70">التوليد والتشغيل</span>
                </li>
                ...
                {{-- السجلات --}}
                @if($canViewRecords)
                <li class="slide has-sub {{ ($isOpen('admin.operation-logs.*') || $isOpen('admin.fuel-efficiencies.*') || $isOpen('admin.maintenance-records.*') || $isOpen('admin.compliance-safeties.*')) ? 'open' : '' }}">
                    <a href="javascript:void(0);" class="side-menu__item {{ ($isActive('admin.operation-logs.*') || $isActive('admin.fuel-efficiencies.*') || $isActive('admin.maintenance-records.*') || $isActive('admin.compliance-safeties.*')) ? 'active' : '' }}">
                        {!! $svgAngle !!}
                ...
                <li class="slide__category mt-3">
                    <span class="side-menu__label text-muted text-xs opacity-70">المشتركون والفوترة</span>
                </li>
                ...
                @if($subsAllowed)
                    ...
                    <li class="slide {{ $isActive('admin.meter-readings.*') }}">
                        <a href="{{ route('admin.meter-readings.index') }}" class="side-menu__item">
                            <svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" width="24" height="24" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 5h-2v6l5.25 3.15.75-1.23-4.75-2.82V7z"/></svg>
                            <span class="side-menu__label">قراءات العدادات</span>
                        </a>
                    </li>
                    @endif
                    @if($canInvoices)
                    <li class="slide {{ $isActive('admin.invoices.*') }}">
                        <a href="{{ route('admin.invoices.index') }}" class="side-menu__item">
                            ...
                            <span class="side-menu__label">الفوترة والتحصيل</span>
                        </a>
                    </li>
                    @endif
                    @if($canSubAccount)
                    <li class="slide {{ $isActive('admin.subscriber-account.*') }}">
                        <a href="{{ route('admin.subscriber-account.index') }}" class="side-menu__item">
                            <svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" width="24" height="24" viewBox="0 0 24 24"><path d="M3 6h18v2H3V6zm0 5h18v2H3v-2zm0 5h12v2H3v-2z"/></svg>
                            <span class="side-menu__label">حساب المشترك</span>
                        </a>
                    </li>
                    @endif
                    @if($canInvoiceReports)
                    <li class="slide {{ $isActive('admin.invoice-reports.*') }}">
                        <a href="{{ route('admin.invoice-reports.index') }}" class="side-menu__item">
                            <svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" width="24" height="24" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z"/></svg>
                            <span class="side-menu__label">تقارير الفوترة</span>
                        </a>
                    </li>
                    @endif
                @endif

                {{-- إعدادات الفوترة (التعرفة + خصم الموظفين + الحد الأدنى) --}}
                @if($showBillingSettings)
                <li class="slide has-sub {{ $billingSettingsOpen ? 'open' : '' }}">
                    <a href="javascript:void(0);" class="side-menu__item {{ $billingSettingsActive ? 'active' : '' }}">
                        {!! $svgAngle !!}
                        <svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" width="24" height="24" viewBox="0 0 24 24"><path d="M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z"/></svg>
                        <span class="side-menu__label">إعدادات الفوترة</span>
                    </a>
                    <ul class="slide-menu child1" style="{{ $billingSettingsOpen ? 'display:block' : '' }}">
                        @can('viewAny', App\Models\ElectricityTariffPrice::class)
                        @if($u->isCompanyOwner() && $u->ownedOperators()->first())
                        <li class="slide"><a href="{{ route('admin.operators.tariff-prices.index', $u->ownedOperators()->first()) }}" class="side-menu__item {{ $isActive('admin.operators.tariff-prices.*') }}">أسعار التعرفة</a></li>
                        @else
                        <li class="slide"><a href="{{ route('admin.electricity-tariff-prices.index') }}" class="side-menu__item {{ $isActive('admin.electricity-tariff-prices.*') }}">أسعار التعرفة</a></li>
                        @endif
                        @endcan
                        @if($u->isSuperAdmin() || $u->hasPermission('employee_discount_rates.view'))
                        <li class="slide"><a href="{{ route('admin.employee-discount-rates.index') }}" class="side-menu__item {{ $isActive('admin.employee-discount-rates.*') }}">خصم الموظفين</a></li>
                        @endif
                        @if($subsAllowed && $canMinCharge)
                ...
                @endif
                @endif {{-- end المشتركون والفوترة --}}

                {{-- ══════════════════════════════════════
                   التواصل والخدمات — حسب الصلاحيات (الشكاوى للجميع)
                   ══════════════════════════════════════ --}}
                <li class="slide__category mt-3">
                    <span class="side-menu__label text-muted text-xs opacity-70">التواصل والخدمات</span>
                </li>
                ...
                <li class="slide has-sub {{ ($isOpen('admin.welcome-messages.*') || $isOpen('admin.sms-templates.*')) ? 'open' : '' }}">
                    <a href="javascript:void(0);" class="side-menu__item {{ ($isActive('admin.welcome-messages.*') || $isActive('admin.sms-templates.*')) ? 'active' : '' }}">
                        {!! $svgAngle !!}
                        <svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" width="24" height="24" viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/></svg>
                        <span class="side-menu__label">قوالب الرسائل</span>
                    </a>
                ...
                @if($u->isSuperAdmin())
                @php
                    $isSystemSettingsOpen = $isOpen('admin.settings.*') || $isOpen('admin.constants.*') || $isOpen('admin.versions.*') || $isOpen('admin.logs.*') || $isOpen('admin.backups.*');
                    $isSystemSettingsActive = $isActive('admin.settings.*') || $isActive('admin.constants.*') || $isActive('admin.versions.*') || $isActive('admin.logs.*') || $isActive('admin.backups.*');
                @endphp
                <li class="slide has-sub {{ $isSystemSettingsOpen ? 'open' : '' }}">
                    ...
                    <ul class="slide-menu child1" style="{{ ($show('admin.settings.*') || $show('admin.constants.*') || $show('admin.versions.*') || $show('admin.logs.*') || $show('admin.backups.*')) ? 'display:block' : '' }}">
                        ...
                        <li class="slide"><a href="{{ route('admin.backups.index') }}" class="side-menu__item {{ $isActive('admin.backups.*') }}"><i class="bi bi-cloud-arrow-down me-1 text-info"></i>النسخ الاحتياطي</a></li>
