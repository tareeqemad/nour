        ...
        $(document).ready(function() {
            ...
            @if(auth()->user()->isSuperAdmin())
                ...
                $operatorSelect.on('change', async function() {
                    ...
                    // إذا كانت القيمة 0 أو فارغة، لا نرسل request
                    if (!operatorId || operatorId == '0' || operatorId === '0') {
                        $generationUnitSelect.empty().append('<option value="0">-- اختر وحدة التوليد --</option>').prop('disabled', true);
                        return; // إيقاف التنفيذ هنا - لا يتم إرسال request
                    }
                ...
                $generationUnitSelect.on('change', async function() {
                    ...
                    // إذا كانت القيمة 0 أو فارغة، لا نرسل request
                    if (!generationUnitId || generationUnitId == '0' || generationUnitId === '0') {
            ...
            async function loadTariffPrice() {
                ...
                // إذا كانت القيمة 0 أو فارغة، لا نرسل request
                if (!operatorId || operatorId == '0' || operatorId === '0' || !operationDate || !tariffPriceInput) {
                    return; // إيقاف التنفيذ - لا يتم إرسال request
                }
