@if(!empty($append))
                ...
                @if($subscriber->generationUnits->count() > 0)
                    <div class="d-flex flex-wrap gap-1">
                        ...
                        @if($subscriber->generationUnits->count() > 2)
                            <span class="badge bg-secondary">+{{ $subscriber->generationUnits->count() - 2 }}</span>
                        @endif
                    </div>
                @else
                    <span class="text-muted">-</span>
            ...
            <td class="text-center">
                @php
                    $statusClass = match((int)$subscriber->subscription_status) {
                        1 => 'bg-success', 2 => 'bg-warning', 3 => 'bg-danger', default => 'bg-secondary',
                    };
                @endphp
                <span class="badge {{ $statusClass }} px-3 py-2">{{ $subscriber->subscription_status_name }}</span>
            </td>
            <td class="text-end">
    ...
    @endforeach
@else
    {{-- وضع كامل: الجدول + عداد النتائج --}}
    @if($subscribers->count() > 0)
        <div class="d-flex justify-content-between align-items-center mb-3">
            <small class="text-muted">
                <i class="bi bi-people me-1"></i>
                إجمالي النتائج: <strong id="subscribersCount">{{ $subscribers->total() }}</strong> مشترك
            </small>
        </div>
        <div class="table-responsive">
            <table class="table table-hover align-middle table-striped mb-0" id="subscribersTable">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="fw-semibold"><i class="bi bi-person me-1"></i>اسم المشترك</th>
                        <th class="fw-semibold"><i class="bi bi-credit-card me-1"></i>رقم الهوية</th>
                        <th class="fw-semibold"><i class="bi bi-telephone me-1"></i>الجوال</th>
                        <th class="fw-semibold"><i class="bi bi-lightning-charge me-1"></i>وحدات التوليد</th>
                        <th class="text-center fw-semibold"><i class="bi bi-funnel me-1"></i>الحالة</th>
                        <th class="text-end fw-semibold"><i class="bi bi-gear me-1"></i>الإجراءات</th>
                    </tr>
                </thead>
        ...
        <div class="text-center py-5">
            ...
            @can('create', App\Models\Subscriber::class)
                <a href="{{ route('admin.subscribers.create') }}" class="btn btn-primary mt-2">
                    <i class="bi bi-plus-lg me-1"></i>إضافة مشترك جديد
                </a>
            @endcan
        </div>
    @endif
