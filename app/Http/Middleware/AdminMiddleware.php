...
class AdminMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (! $user->isSuperAdmin() && ! $user->isAdmin() && ! $user->isEnergyAuthority() && ! $user->isGeneralAccountant() && ! $user->isCompanyOwner() && ! $user->isEmployee() && ! $user->isTechnician() && ! $user->isCivilDefense() && ! $user->hasCustomRole()) {
            abort(403);
        }
